...
export function DeleteSaleSheet({ sale, product, onClose, onConfirm }: { sale?: SalesRecord; product?: LedgerProduct; onClose: () => void; onConfirm: () => void }) {
  ...
  return <div className="sheet-backdrop" role="presentation" onMouseDown={onClose}><section className="pricing-sheet sale-delete-sheet" role="dialog" aria-modal="true" aria-label="删除销售记录" onMouseDown={(event) => event.stopPropagation()}><div className="sheet-grabber" /><header className="sheet-header"><div><span className="eyebrow">销售纠错</span><h2>删除这笔销售？</h2></div><button className="icon-button" onClick={onClose}>×</button></header><div className="sale-refund-origin"><b>{product?.name ?? "已归档商品"}</b><small>{sale.date} · {sale.quantity} 件 · {formatCurrency(sale.quantity * sale.unitPrice)}</small>{refundAmount > 0 && <small>已退款 {formatCurrency(refundAmount)}，将一并删除退款记录。</small>}</div><div className="refund-impact"><span>删除后将</span><p>移除这笔销售及其关联收款、退款和成本结转；若已启用库存，会恢复未通过退款恢复的库存 {Math.max(sale.quantity - restockedQuantity, 0)} 件。</p></div><button className="danger-action sheet-action" onClick={onConfirm}>确认删除销售</button></section></div>;
}
  ...
  return (
    <div className="page-content business-content">
      ...
      <section className={`cash-flow-card ${isRefreshing ? "is-refreshing" : ""}`}>
        <div className="cash-flow-copy"><span>现金流出 <button className="micro-info" aria-label="查看现金流压力说明" aria-expanded={showCashDetails} onClick={() => setShowCashDetails((current) => !current)}><Info size={13} /></button></span><h2>{formatCurrency(summary.cashOutflow)}</h2>{summary.cashOutflow > 0 ? <p>本金 {formatCurrency(summary.principalRepayment)} · 利息 {formatCurrency(summary.financingCosts)}</p> : <p className="cash-flow-empty">暂无支出</p>}{showCashDetails && <div className="cash-flow-detail" role="status">本金只影响现金；利息计入成本。</div>}<button className="text-action cash-records-link" onClick={onCashRecords}>查看流水明细<ChevronRight size={14} /></button></div><div className="cash-orbit" aria-hidden="true"><CircleDollarSign size={32} /><span>{isRefreshing ? "更新中" : "本期"}</span></div>
      </section>
      ...
      {periodSales.length > 0 && <section className="sales-history-card"><div className="section-heading compact"><div><span className="eyebrow">销售记录</span><h2>退款、撤销或删除</h2></div></div><div className="sales-history-list">{periodSales.map((sale) => { const product = products.find((item) => item.id === sale.productId); const remaining = getRefundableSaleQuantity(sale); const refunded = (sale.refunds ?? []).reduce((sum, refund) => sum + refund.amount, 0); return <article className="sales-history-row" key={sale.id}><div><b>{product?.name ?? "已归档商品"}</b><small>{sale.date} · {sale.quantity} 件 · {formatCurrency(sale.quantity * sale.unitPrice)}{refunded > 0 ? ` · 已退 ${formatCurrency(refunded)}` : ""}</small></div><div className="sales-history-actions">{remaining > 0 ? <button onClick={() => onRefund(sale.id)}>退款/撤销 <ChevronRight size={14} /></button> : <span className="sale-voided">已撤销</span>}<button className="sale-delete-action" aria-label={`删除${product?.name ?? "销售"}记录`} onClick={() => onDeleteSale(sale.id)}><Trash2 size={14} />删除</button></div></article>; })}</div></section>}
      {(hasTrendData || summary.salesCount > 0) && <section className={`chart-card ${isRefreshing ? "is-refreshing" : ""}`} aria-label="现金收付趋势图表"><div className="chart-heading"><div><span className="eyebrow">现金流</span><h2>现金收付</h2></div><span className="legend"><i />收款 <i className="green" />付款</span></div>{isRefreshing && <div className="chart-loading" role="status" aria-live="polite"><span className="loading-sweep" />更新中</div>}{hasTrendData ? <><div className="bar-chart" role="list" aria-label="按日期查看现金收款和付款">{summary.dailySeries.map((item) => <button type="button" className={`bar-pair ${activeItem?.label === item.label ? "is-active" : ""}`} key={item.label} onMouseEnter={() => setActiveLabel(item.label)} onFocus={() => setActiveLabel(item.label)} onClick={() => setActiveLabel(item.label)} aria-label={`${item.label}，收款${formatCurrency(item.income)}，付款${formatCurrency(item.expenses)}`} role="listitem"><span className="income-bar" aria-hidden="true" style={{ height: `${Math.max((item.income / maxValue) * 100, item.income ? 8 : 0)}%` }} /><span className="expense-bar" aria-hidden="true" style={{ height: `${Math.max((item.expenses / maxValue) * 100, item.expenses ? 8 : 0)}%` }} /></button>)}</div><div className="chart-labels">{summary.dailySeries.map((item) => <button type="button" className={activeItem?.label === item.label ? "is-active" : ""} onClick={() => setActiveLabel(item.label)} key={item.label}>{item.label}</button>)}</div>{activeItem && <div className="chart-tooltip" role="status"><b>{activeItem.label}</b><span><i className="income-dot" />收款 {formatCurrency(activeItem.income)}</span><span><i className="expense-dot" />付款 {formatCurrency(activeItem.expenses)}</span></div>}</> : null}</section>}
      ...
      <section className="ledger-lines">
        <LineItem icon={<Coins size={18} />} label="已付现金（不含本金）" value={formatCurrency(summary.expenses)} width={`${summary.expenses ? Math.min(summary.expenses / Math.max(summary.income, summary.expenses) * 100, 100) : 0}%`} color="blue" />
