...
import {   applyIndustryTemplate, applyQuickCost,
  ...
  formatBusinessPeriod, getActiveCategories, getMonthlyIndirectPlan, calculateUnitCost, getBusinessDate, getBusinessPeriod, INDUSTRY_TEMPLATES, AllocationMethod, HiddenCostItem, IndustryKey, LedgerData, initializeIndustryLedger, LedgerProduct, MonthlyFixedCosts, MonthlyIndirectCostPlan, ProductAllocationInput, loadLedger, makeBomVersionSnapshot, makeId, Material, normalizeLedger, persistLedger, recalculateProduct, renameLedgerCategory, SaleRefund, SalesRecord, summarizeLedger } from "@/lib/ledgerStore";
...
export default function Home() {
  ...
  return (
    <div className="app-shell">
      ...
      {showMonthlyAllocation && <MonthlyAllocationSheet period={selectedPeriod} products={activeProducts} initialPlan={getMonthlyIndirectPlan(ledger, selectedPeriod)} onClose={() => setShowMonthlyAllocation(false)} onSave={(plan) => { setLedger((current) => { const monthlyIndirectPlans = [...(current.costs.monthlyIndirectPlans ?? []).filter((item) => item.period !== plan.period), plan]; const costs = { ...current.costs, monthlyIndirectPlans, allocationPeriod: plan.period }; const products = applyMonthlyIndirectPlan(current.products, plan, current.materials); const next = { ...current, costs, products }; persistLedger(next); return next; }); setShowMonthlyAllocation(false); notify(`已保存${formatBusinessPeriod(plan.period)}分摊；未来单件完整成本已更新`); }} />}
