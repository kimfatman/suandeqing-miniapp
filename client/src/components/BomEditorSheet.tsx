...
export function BomEditorSheet({ product, materials, categories = [], costLabel = "商品配方 BOM", costAction = "加入配方", costEmpty = "还没有配方材料，先从下面加一项。", onClose, onSave }: BomEditorSheetProps) {
  ...
  const addItem = () => {
    if (addMode === "material" && materialId) {
      ...
      setItems((current) => [...current, { id: `custom-${Date.now()}`, materialId: "", quantity: 1, customName: "", customUnit: "项", customUnitCost: 0 }]);
    }
  ...
  const save = () => {
    ...
    onSave(items, { lossRate: parsedLossRate, batchYield: parsedBatchYield, costCategory: costCategory || undefined, costSnapshot });
  };
  ...
  return <div className="sheet-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="pricing-sheet bom-sheet" role="dialog" aria-modal="true" aria-label={`编辑${costLabel}`} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <header className="sheet-header"><div><span className="eyebrow">{costLabel}</span><h2>{product.name} · 成本有来路</h2></div><button className="icon-button" onClick={onClose} aria-label="关闭"><X size={19} /></button></header>
      <section className="bom-total"><span>当前材料与成本明细</span><strong>{formatCurrency(directCost)}</strong><p>已按损耗 {lossRate || 0}%、本批出成 {batchYield || 0} 份折算；包装 {formatCurrency(product.packaging)} ＋ 直接人工 {formatCurrency(product.directLabor)} 会在保存后一起计入。</p></section>
      ...
      {categories.length > 0 && <label className="field-block bom-category-field"><span>商品成本归类</span><select value={costCategory} onChange={(event) => setCostCategory(event.target.value)}>{categories.map((category) => <option key={category} value={category}>{category}</option>)}</select><small>停用的成本项目不会出现在新记账选择中，但历史流水仍保留。</small></label>}
      <section className="bom-list">{items.length ? items.map((item) => {
        const material = materials.find((entry) => entry.id === item.materialId);
        if (item.customName !== undefined) return <article className="bom-row custom-bom-row" key={item.id}><div className="bom-dot custom" /><div className="custom-cost-fields"><label><span>项目名称</span><input aria-label="自定义成本名称" value={item.customName} placeholder="例如：平台服务费" onChange={(event) => updateCustom(item.id, { customName: event.target.value })} /></label><div className="custom-cost-inline"><label><span>单位</span><input aria-label="自定义成本单位" value={item.customUnit ?? ""} placeholder="项" onChange={(event) => updateCustom(item.id, { customUnit: event.target.value })} /></label><label><span>单价</span><input aria-label="自定义成本单价" type="number" min="0.01" step="0.01" value={item.customUnitCost || ""} onChange={(event) => updateCustom(item.id, { customUnitCost: event.target.value === "" ? 0 : Number(event.target.value) })} /></label></div></div><label className="custom-quantity"><span>数量</span><input aria-label="自定义成本数量" type="number" min="0.01" step="0.01" value={item.quantity || ""} onChange={(event) => updateQuantity(item.id, event.target.value)} /></label><button onClick={() => { setItems((current) => current.filter((entry) => entry.id !== item.id)); setCostSnapshot(undefined); }} aria-label="删除自定义成本"><Trash2 size={16} /></button></article>;
      ...
      <div className="bom-add"><select aria-label="新增成本明细类型" value={addMode} onChange={(event) => setAddMode(event.target.value as "material" | "custom")}><option value="material">已有材料</option><option value="custom">自定义成本项目</option></select>{addMode === "material" && <select aria-label="选择材料" value={materialId} onChange={(event) => setMaterialId(event.target.value)}>{materials.map((material) => <option key={material.id} value={material.id}>{material.name}</option>)}</select>}<button onClick={addItem}><Plus size={16} /> {addMode === "custom" ? "新增自定义项目" : costAction}</button></div>
      <details className="calculation-details"><summary>成本如何变化？ <ChevronDown size={16} /></summary><p>保存后会生成新的商品成本版本。之前的售价和历史经营记录不会被当前成本明细覆盖。</p>{product.bomVersions?.length ? <div className="bom-history">{product.bomVersions.map((version) => <button type="button" key={version.id} onClick={() => { setItems(version.items); setLossRate(String(version.lossRate)); setBatchYield(String(version.batchYield)); setCostSnapshot({ materialUnitCosts: version.materialUnitCosts, packaging: version.packaging, directLabor: version.directLabor, directCost: calculateBomVersionDirectCost(version) }); setValidationError(null); }}>恢复 {version.effectiveFrom} · 历史成本{formatCurrency(calculateBomVersionDirectCost(version))}</button>)}</div> : <small>当前还没有历史版本；第一次保存后会生成快照。</small>}</details>
