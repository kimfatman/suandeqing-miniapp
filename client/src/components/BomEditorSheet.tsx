...
export function BomEditorSheet({ product, materials, onClose, onSave }: BomEditorSheetProps) {
  ...
  const addItem = () => { if (materialId) { setItems((current) => [...current, { id: `bom-${Date.now()}`, materialId, quantity: 1 }]); setValidationError(null); } };
