...
export function QuickRecordSheet({ categories, onClose, onSave }: QuickRecordSheetProps) {
  ...
  return <div className="sheet-backdrop" role="presentation" onMouseDown={onClose}><section className="pricing-sheet record-sheet" role="dialog" aria-modal="true" aria-label="快速记一笔" onMouseDown={(event) => event.stopPropagation()}><div className="sheet-grabber" /><header className="sheet-header"><div><span className="eyebrow">经营流水</span><h2>记一笔，账就更新。</h2></div><button className="icon-button" onClick={onClose} aria-label="关闭"> <X size={19} /> </button></header><div className="record-type-switch"><button className={type === "expense" ? "active expense" : ""} onClick={() => setType("expense")}><ArrowDownLeft size={16} /> 支出</button><button className={type === "income" ? "active income" : ""} onClick={() => { setType("income"); setCategory("销售收入"); }}><ArrowUpRight size={16} /> 收入</button></div><label className="record-amount"><span>{type === "expense" ? "今天花了多少？" : "今天收了多少？"}</span><div><b>¥</b><input autoFocus type="number" inputMode="decimal" value={amount} placeholder="例如 286" onChange={(event) => setAmount(event.target.value)} /><i>元</i></div></label><div className="record-field-block"><span>归到哪一类账？</span>{type === "income" ? <div className="record-categories">{["销售收入", "其他收入"].map((item) => <button key={item} className={category === item ? "selected" : ""} onClick={() => setCategory(item)}>{item}</button>)}</div> : <><small className="category-group-label">经营支出</small><div className="record-categories">{operatingCategories.map((item) => <button key={item} className={category === item ? "selected" : ""} onClick={() => setCategory(item)}>{item}</button>)}</div><small className="category-group-label">借款与还款</small><div className="record-categories financing-categories">{financingCategories.map((item) => <button key={item} className={category === item ? "selected" : ""} onClick={() => setCategory(item)}>{item}</button>)}</div><p className="record-category-hint">{categoryHint}</p></>}</div><label className="record-note"><span>备注（选填）</span><input value={note} onChange={(event) => setNote(event.target.value)} placeholder="例如：去批发市场取货" /></label><button className="primary-action sheet-action" disabled={!Number.isFinite(amountValue) || amountValue <= 0} onClick={() => onSave({ type, amount: amountValue, category, note })}><Check size={18} /> 保存这笔账</button></section></div>;
}
