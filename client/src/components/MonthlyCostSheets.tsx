...
const primaryFixedFields = fixedFields.filter((field) => ["rent", "fullTimeLabor", "utilities", "other"].includes(field.key));
const advancedFixedFields = fixedFields.filter((field) => !["rent", "fullTimeLabor", "utilities", "other"].includes(field.key));
...
export function MonthlyAllocationSheet({ period, products, initialPlan, onClose, onSave, onDelete }: { period: string; products: LedgerProduct[]; initialPlan?: MonthlyIndirectCostPlan; onClose: () => void; onSave: (plan: MonthlyIndirectCostPlan) => void; onDelete?: () => void }) {
  ...
  const [showAdvanced, setShowAdvanced] = useState(() => Boolean(initialPlan && (initialPlan.fixedCosts.propertyInternet > 0 || initialPlan.fixedCosts.softwareServer > 0 || initialPlan.fixedCosts.officeMisc > 0 || initialPlan.fixedCosts.equipment.length > 0 || initialPlan.method !== "output" || initialPlan.costTiming === "prorated" || initialTiming.effectiveFrom !== range.start || initialTiming.effectiveTo !== range.end)));
  ...
  const advance = () => {
    setError(null);
    if (step === 1 && validateExpenseStep()) setStep(2);
    if (step === 2 && validateBasisStep()) setStep(3);
  };

  return <div className="sheet-backdrop" role="presentation" onMouseDown={onClose}><section className="pricing-sheet monthly-allocation-sheet" role="dialog" aria-modal="true" aria-label="月度成本分摊" onMouseDown={(event) => event.stopPropagation()}><div className="sheet-grabber" /><header className="sheet-header"><div><span className="eyebrow">{period}</span><h2>月度成本分摊</h2></div><button className="icon-button" onClick={onClose}>×</button></header><div className="cost-setting-note"><Info size={17} /><p>先完成三步基础分摊；设备、有效日期、工时、销售额与权重仍可在高级设置中完整使用。已发生销售继续使用当时冻结的成本快照。</p></div><div className="allocation-stepper" aria-label="月度分摊步骤">{([1, 2, 3] as const).map((item) => <span className={item === step ? "active" : item < step ? "done" : ""} key={item}><b>{item}</b><small>{item === 1 ? "本期费用" : item === 2 ? "怎么分" : "确认商品"}</small></span>)}</div>
    {step === 1 && <section className="monthly-section"><div className="monthly-section-heading"><b>本期费用</b><span>先填最常见的四项</span></div><div className="monthly-fixed-grid">{primaryFixedFields.map((field) => <label className="field-block" key={field.key}><span>{field.label}<small>{field.hint}</small></span><NumberInput ariaLabel={`${field.label}月费`} value={fixedCosts[field.key]} onChange={(value) => setFixedCosts((current) => ({ ...current, [field.key]: value }))} /></label>)}</div><button className="allocation-advanced-toggle" type="button" onClick={() => setShowAdvanced((current) => !current)}>{showAdvanced ? "收起高级设置" : "设备折旧、时间比例和其他费用"}</button>{showAdvanced && <div className="allocation-advanced-panel"><section><div className="monthly-section-heading"><b>其他费用</b><span>按需填写</span></div><div className="monthly-fixed-grid">{advancedFixedFields.map((field) => <label className="field-block" key={field.key}><span>{field.label}<small>{field.hint}</small></span><NumberInput ariaLabel={`${field.label}月费`} value={fixedCosts[field.key]} onChange={(value) => setFixedCosts((current) => ({ ...current, [field.key]: value }))} /></label>)}</div></section><section><div className="monthly-section-heading"><b>本期生效时间</b><span>按业务日期匹配</span></div><div className="allocation-timing-grid"><label className="field-block"><span>开始日期</span><input aria-label="分摊开始日期" type="date" min={range.start} max={range.end} value={effectiveFrom} onChange={(event) => setEffectiveFrom(event.target.value)} /></label><label className="field-block"><span>结束日期</span><input aria-label="分摊结束日期" type="date" min={range.start} max={range.end} value={effectiveTo} onChange={(event) => setEffectiveTo(event.target.value)} /></label></div><div className="timing-mode"><button className={costTiming === "fullMonth" ? "selected" : ""} onClick={() => setCostTiming("fullMonth")}><b>本期实际费用</b><small>填入金额直接计入，不再按天折算</small></button><button className={costTiming === "prorated" ? "selected" : ""} onClick={() => setCostTiming("prorated")}><b>整月预算按天折算</b><small>按有效 {timing.effectiveDays} / {timing.daysInPeriod} 天计入</small></button></div></section><section className="equipment-list"><div className="monthly-section-heading"><b>设备折旧</b><button type="button" onClick={addEquipment}><Plus size={14} />新增设备</button></div>{fixedCosts.equipment.map((equipment) => <div className="equipment-row" key={equipment.id}><input aria-label="设备名称" value={equipment.name} onChange={(event) => setFixedCosts((current) => ({ ...current, equipment: current.equipment.map((item) => item.id === equipment.id ? { ...item, name: event.target.value } : item) }))} /><NumberInput ariaLabel={`${equipment.name}采购价`} value={equipment.purchasePrice} onChange={(value) => setFixedCosts((current) => ({ ...current, equipment: current.equipment.map((item) => item.id === equipment.id ? { ...item, purchasePrice: value } : item) }))} /><NumberInput ariaLabel={`${equipment.name}使用月数`} suffix="月" min={1} step={1} value={equipment.usefulLifeMonths} onChange={(value) => setFixedCosts((current) => ({ ...current, equipment: current.equipment.map((item) => item.id === equipment.id ? { ...item, usefulLifeMonths: value } : item) }))} /><b>{formatCurrency(calculateEquipmentDepreciation(equipment))}/月</b><button aria-label={`删除${equipment.name}`} onClick={() => setFixedCosts((current) => ({ ...current, equipment: current.equipment.filter((item) => item.id !== equipment.id) }))}><Trash2 size={15} /></button></div>)}</section></div>}<div className="monthly-total"><span>{costTiming === "prorated" ? `本期计入（${timing.effectiveDays}/${timing.daysInPeriod} 天）` : "本期计入间接成本"}</span><strong>{formatCurrency(total)}</strong>{costTiming === "prorated" && <small>整月预算 {formatCurrency(fullMonthTotal)}</small>}</div></section>}
    {step === 2 && <section className="monthly-section"><div className="monthly-section-heading"><b>怎么分</b><span>默认按产量即可开始</span></div><div className="allocation-methods">{methodOptions.map((option) => <button className={method === option.key ? "selected" : ""} key={option.key} onClick={() => { setMethod(option.key); setError(null); }}><b>{option.label}</b><small>{option.note}</small></button>)}</div>{method === "output" && <p className="allocation-step-hint">下一步填写各商品本期产量，系统会按产量比例分摊。若商品差异很大，再选择工时或销售额。</p>}{method === "hours" && <label className="field-block monthly-hours"><span>本期总生产工时<small>生效日期内所有商品制作工时合计</small></span><NumberInput ariaLabel="本期总生产工时" suffix="小时" min={0.01} value={totalProductionHours} onChange={setTotalProductionHours} /></label>}{method === "revenue" && <p className="allocation-step-hint">下一步填写每种商品的本期预计销售额与产量；此数据仅用于分摊与定价预估，不写入已发生收入。</p>}</section>}
    {step === 3 && <section className="monthly-section"><div className="monthly-section-heading"><b>确认商品</b><span>{showAdvanced ? "可设置权重" : "填写预计产量即可"}</span></div><div className="allocation-product-list">{products.map((product) => { const input = inputs.find((item) => item.productId === product.id)!; const allocation = allocations[product.id]; return <article className="allocation-product-row" key={product.id}><div className="allocation-product-name"><b>{product.name}</b><small>直接成本 {formatCurrency(product.direct)}</small></div><div className="allocation-inputs"><label><span>本期产量</span><NumberInput ariaLabel={`${product.name}本期产量`} suffix="件" min={0} value={input.outputQuantity} onChange={(value) => setInput(product.id, "outputQuantity", value)} /></label>{method === "hours" && <label><span>单件工时</span><NumberInput ariaLabel={`${product.name}单件工时`} suffix="小时" min={0} value={input.unitHours} onChange={(value) => setInput(product.id, "unitHours", value)} /></label>}{method === "revenue" && <label><span>本期销售额</span><NumberInput ariaLabel={`${product.name}本期销售额`} value={input.salesAmount} onChange={(value) => setInput(product.id, "salesAmount", value)} /></label>}{showAdvanced && <label><span>权重</span><NumberInput ariaLabel={`${product.name}分摊权重`} suffix="倍" min={0.1} step={0.1} value={input.weight} onChange={(value) => setInput(product.id, "weight", value)} /></label>}</div><div className="allocation-product-result"><span>每件分摊</span><b>{formatCurrency(allocation?.unitIndirectCost ?? 0)}</b><small>完整成本 {formatCurrency(product.direct + (allocation?.unitIndirectCost ?? 0))}</small></div></article>; })}</div>{!showAdvanced && <button type="button" className="allocation-advanced-toggle inline" onClick={() => setShowAdvanced(true)}>需要按商品差异调整？设置权重</button>}<div className="allocation-check"><span>预计分配 {formatCurrency(allocatedTotal)} / {formatCurrency(total)}</span>{method === "hours" && Math.abs(allocatedTotal - total) > 0.01 && <small>工时法未分配部分代表尚未录入的本期生产工时。</small>}</div></section>}{error && <p className="form-error" role="alert">{error}</p>}<div className="allocation-footer">{step > 1 && <button className="secondary-action" type="button" onClick={() => { setStep((current) => current === 3 ? 2 : 1); setError(null); }}>上一步</button>}{step < 3 ? <button className="primary-action sheet-action" type="button" onClick={advance}>下一步</button> : <button className="primary-action sheet-action" onClick={save}><Check size={16} />保存本期分摊并更新未来成本</button>}</div>{initialPlan && onDelete && (confirmDelete ? <div className="data-confirmation"><b>删除本月分摊？</b><p>本月房租、人工、折旧等设置将移除；已发生销售继续使用原成本快照。</p><div><button className="secondary-action" onClick={() => setConfirmDelete(false)}>取消</button><button className="danger-action" onClick={onDelete}>确认删除</button></div></div> : <button className="danger-action sheet-action" onClick={() => setConfirmDelete(true)}>删除本月分摊</button>)}</section></div>;
}
